...
template SMTProcessor(nLevels) {
    ...
    for (i=0; i<nLevels * 4; i++) smtLevIns.siblings[i] <== siblings[i];
    smtLevIns.enabled <== enabled;

    component newKeyQuins = Dec2Quin(111);
    newKeyQuins.in <== key;
    component oldKeyQuins = Dec2Quin(111);
    oldKeyQuins.in <== oldKey;

    component eqs[nLevels];
    for (i=0; i<nLevels; i++) {
        eqs[i] = IsEqual();
        eqs[i].in[0] <== newKeyQuins.out[i];
        eqs[i].in[1] <== oldKeyQuins.out[i];
    }
    ...
    for (i=nLevels-1; i != -1; i--) {
        ...
        levels[i].index <== newKeyQuins.out[i];
        levels[i].oldIndex <== oldKeyQuins.out[i];
        if (i==nLevels-1) {
            levels[i].oldChild <== 0;
            levels[i].newChild <== 0;
        } else {
