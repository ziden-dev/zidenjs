...
class Eddsa {
  ...
  prv2pub(prv) {
    ...
    let s = getFF().Scalar.fromRprLE(sBuff, 0, 32);
    const A = this.babyJub.mulPointEscalar(this.babyJub.Base8, getFF().Scalar.shr(s, 3));
    return A;
  }
  ...
  signPoseidon(prv, msg) {
    const F = this.babyJub.F;
    const sBuff = this.pruneBuffer(createBlakeHash('blake512').update(Buffer.from(prv)).digest());
    const s = getFF().Scalar.fromRprLE(sBuff, 0, 32);
    const A = this.babyJub.mulPointEscalar(this.babyJub.Base8, getFF().Scalar.shr(s, 3));
    ...
    let r = getFF().Scalar.mod(getFF().Scalar.fromRprLE(rBuff, 0, 64), this.babyJub.subOrder);
    const R8 = this.babyJub.mulPointEscalar(this.babyJub.Base8, r);

    const hm = this.poseidon([R8[0], R8[1], A[0], A[1], msg]);
    const hms = getFF().Scalar.e(this.babyJub.F.toObject(hm));
    const S = getFF().Scalar.mod(getFF().Scalar.add(r, getFF().Scalar.mul(hms, s)), this.babyJub.subOrder);
    return {
      R8: R8,
      S: S,
    };
  ...
  verifyPoseidon(msg, sig, A) {
    ...
    let Pright = this.babyJub.mulPointEscalar(A, getFF().Scalar.mul(hms, 8));
  ...
  packSignature(sig) {
    ...
    buff.set(R8p, 0);
    const Sp = getFF().Scalar.toRprLE(buff, 32, sig.S, 32);
    return buff;
  }
  ...
  unpackSignature(sigBuff) {
    return {
      ...
      S: getFF().Scalar.fromRprLE(sigBuff, 32, 32),
    };
  }
}
