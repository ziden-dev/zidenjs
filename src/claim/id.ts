 ...
 * @param {BigInt} clr claims tree root
 * @param {BigInt} aur auth tree root
 * @returns {ArrayLike<number>} idenState
 */
export function idenState(clr: BigInt, aur: BigInt): ArrayLike<number> {
  return getZidenParams().hasher([clr, aur]);
}
