...
import { Entry, newClaim, schemaHashFromBigInt, withExpirationDate, withIndexData, withSlotData } from './claim/entry.js';
...
describe('test state transition', async () => {
  ...
  it('2nd state transition', async () => {
    ...
    const w2 = await stateTransitionWitnessWithPrivateKeyAndHiHvs(
      ...
      state,
      [],
      [[claim3.hiRaw(), claim3.hvRaw()], [claim4.hiRaw(), claim4.hvRaw()]],
      [],
  ...
  it('3rd state transition', async () => {
    witness = await stateTransitionWitnessWithPrivateKey(priv1, auth1, state, [auth3], [claim5], [auth1.authHi, auth2.authHi], [claim3.getRevocationNonce()]);
  ...
  it('benchmark proving time', async () => {
    await groth16.fullProve(
      witness,
      'src/circom_test/stateTransition.wasm',
      'src/circom_test/stateTransition.zkey'
    );
  }).timeout(100000);
