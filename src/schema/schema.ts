import { Entry, newClaim, schemaHashFromBigInt, withExpirationDate, withFlagExpirable, withFlagUpdatable, withID, withIndexData, withValueData } from '../claim/entry.js';
import { Hasher, SnarkField } from '../global.js';
import { bitsToNum, bufferToFloat, bufferToHex, floatToBuffer, getPartialValue, hexToBuffer, numToBits, setBits, stringToHex } from '../utils.js';
...
export function generateEntry(data: any, schema: Schema, registry: Registry, F: SnarkField, hasher: Hasher): Entry {
    ...
    let indexSlot : Array<BigInt> = dataSlot(data, schema.index, schema.properties, F, hasher);
    let valueSlot : Array<BigInt> = dataSlot(data, schema.value, schema.properties, F, hasher);
...
function dataSlot(data: any, index: Array<string>, properties: any, F: SnarkField, hasher: Hasher) {
    ...
    index.forEach(element => {
        ...
        switch (property["type"]) {
            case "string": // 128 bit
                bitEnd = bitStart + 127;
                let hashData = (F.toObject(hasher([BigInt(stringToHex(data[element]?? ""))]))).toString(2);
...
export function getSchemaHashFromSchema(schema: any, F: SnarkField,hasher: Hasher): string {
    let hashData = (F.toObject(hasher([BigInt(stringToHex(JSON.stringify(schema)))]))).toString(2);
    let bitRemove = hashData.length < 128? 0: hashData.length - 128;
    let hashDataFixed = BigInt("0b" + hashData.slice(0, hashData.length - bitRemove));
