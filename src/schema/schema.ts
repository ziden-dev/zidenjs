...
import { bitsToNum, floatToBuffer, getPartialValue, hexToBuffer, numToBits, setBits, stringToHex } from '../utils.js';
  ...
  try {
    ...
    propertiesKey.forEach((key) => {
      ...
      if (
        ...
        !checkInEnum(propertyId, Slot) ||
        !checkInEnum(propertyType, Type)
      ) {
        return;
      }
  ...
  try {
    ...
    keys.forEach((key) => {
      ...
      if (type != Type.obj) {
        ...
        if (data == undefined) {
          if (schema['@required'].includes(key)) throw 'Invalid data, required ' + key;
        } else {
